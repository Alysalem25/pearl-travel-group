...
router.get("/clients", authorize(PERMISSIONS.MANAGE_CLIENTS), async (req, res, next) => {
  try {
  ...
  try {
    ...
    if (!isOwnerOrAdmin(req.user, user._id)) {
      return res
        .status(403)
        .json({
          error: "Forbidden: You don't have permission to view this user",
        });
    }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const { name, email, password, number, role, permissions, clientInfo } =
        req.body;
...
router.put(
  ...
  async (req, res, next) => {
    try {
      const { name, email, number, role, permissions, clientInfo, workStatus } =
        req.body;
...
router.get("/reviewed/programs", authMiddleware, async (req, res, next) => {
  try {
    ...
    const BookedPrograms = require("../models/BookedPrograms");
    const programs = await BookedPrograms.find(filter)
      .sort({ updatedAt: -1 })
      .populate("program");
    res.json(programs);
  } catch (err) {
...
router.get("/reviewed/cruises", authMiddleware, async (req, res, next) => {
  try {
    ...
    const BookedCruisies = require("../models/BookedCruseies");
    const cruises = await BookedCruisies.find(filter)
      .sort({ updatedAt: -1 })
      .populate("cruise");
    res.json(cruises);
  } catch (err) {
