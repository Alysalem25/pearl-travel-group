...
//     return (
//         <ProtectedRoute requiredPermission="manage_booked_programs">
...
const BookedProgramsPageContent = () => {
    ...
    // Filter bookings
    const filteredBooked = booked.filter((f: BookedProgram) => {
        const searchMatch = 
            (f.userName?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
            (f.userEmail?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
            (f.program?.titleEn?.toLowerCase() || '').includes(searchTerm.toLowerCase())
    ...
    const stats = {
        total: booked.length,
        pending: booked.filter((b: BookedProgram) => b.status === 'pending').length,
        reviewed: booked.filter((b: BookedProgram) => b.status === 'reviewed').length
    }
                ...
                <div className="sticky top-0 z-40 bg-white border-b border-slate-200 shadow-sm backdrop-blur-xl bg-white/80">
                    <div className="px-6 py-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-slate-900 tracking-tight">Booked Programs</h1>
                                <p className="text-xs text-slate-500 font-medium">Manage program reservations</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="relative group">
                                <div className="absolute left-3.5 top-3.5 text-slate-400 transition-colors group-focus-within:text-violet-500">
                                ...
                                <input
                                    ...
                                    className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 focus:bg-white transition-all duration-200 outline-none text-slate-700 placeholder:text-slate-400"
                                />
                            </div>
                            <div className="relative flex items-center gap-3">
                                ...
                                <select
                                    ...
                                    className="flex-1 pl-11 pr-10 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 focus:bg-white transition-all duration-200 outline-none text-slate-700 appearance-none cursor-pointer"
                                >
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                        ...
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-64 gap-4">
                                <div className="w-8 h-8 border-3 border-slate-200 border-t-violet-500 rounded-full animate-spin" />
                            ...
                            <div className="divide-y divide-slate-100">
                                {filteredBooked.map((f: BookedProgram) => (
                                    <div 
                                        key={f._id} 
                                            ...
                                            <div className="flex-1 space-y-4">
                                                ...
                                                <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                                                    <div className="flex items-center gap-2 mb-2">
                                                        ...
                                                        <span className="text-xs font-bold text-violet-600 uppercase tracking-wider">Program Details</span>
                                                    </div>
                                                    <p className="font-semibold text-slate-900">
                                                        {f.program?.titleEn} 
                                                        {f.program?.titleAr && (
                                                            <span className="text-slate-500 font-normal"> • {f.program.titleAr}</span>
                                                        )}
                                                    </p>
                                                </div>
