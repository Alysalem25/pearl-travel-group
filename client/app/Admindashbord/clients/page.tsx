...
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import apiClient, { api } from '@/lib/api'
import { User, ChevronRight, Search, Filter, X, Phone, Mail, MapPin, CreditCard, FileText, Shield, UserCheck, Eye, Trash2, Plus, ChevronDown, ChevronUp, Edit3, Save } from 'lucide-react'
import { useAuth } from '@/context/AuthContext'
import { ProtectedRoute } from '@/components/ProtectedRoute';
import { PERMISSIONS } from '@/lib/permissionConstants';
import Link from 'next/link'
...
const UsersPageContent = () => {
    ...
    const startEdit = (user: UserData) => {
        ...
        setClientInfo({
            ...
            dateOfBirth: user.clientInfo?.dateOfBirth || '',
            passportIssueDate: user.clientInfo?.passportIssueDate || '',
            passportExpiryDate: user.clientInfo?.passportExpiryDate || '',
            destination: user.clientInfo?.destination || ''
        })
            ...
            <div className="flex-1 overflow-auto bg-slate-50">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">Manage clients and their information</p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="px-6 lg:px-8 py-5">
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4">
                        ...
                        <div className="mt-4 flex items-center justify-between">
                            <span className="text-sm text-slate-500">
                                Showing <span className="font-semibold text-slate-900">{filteredUsers.length}</span> of <span className="font-semibold text-slate-900">{clients.length}</span> clients
                            </span>
                            {isLoading && (
                                <span className="text-sm text-blue-600 animate-pulse">Loading...</span>
                            )}
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <h2 className="text-lg font-bold text-slate-900">
                                    {editingUser ? `Edit Client: ${editingUser.name}` : 'Add New Client'}
                                </h2>
                                {editingUser && (
                                    ...
                                    >
                                        {(addUserMutation.isPending || updateUserMutation.isPending) ? (
                                            ...
                                            <>
                                                ...
                                                {editingUser ? 'Update Client' : 'Add Client'}
                                            </>
                                        )}
                                    </button>
                                </div>
                                            ...
                                            {expandedUser === user._id && user.clientInfo && (
                                                <div className="mt-4 ml-[72px] bg-slate-50 rounded-xl p-4 border border-slate-200 animate-in slide-in-from-top-1 duration-200">
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                        ...
                                                        {user.clientInfo.dateOfBirth && (
                                                            <div className="flex items-start gap-2">
                                                                <FileText className="w-4 h-4 text-slate-400 mt-0.5 flex-shrink-0" />
                                                                <div>
                                                                    <p className="text-xs font-medium text-slate-500">Date of Birth</p>
                                                                    <p className="text-sm text-slate-800">{new Date(user.clientInfo.dateOfBirth).toLocaleDateString()}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {user.clientInfo.passportIssueDate && (
                                                            <div className="flex items-start gap-2">
                                                                <FileText className="w-4 h-4 text-slate-400 mt-0.5 flex-shrink-0" />
                                                                <div>
                                                                    <p className="text-xs font-medium text-slate-500">Passport Issue Date</p>
                                                                    <p className="text-sm text-slate-800">{new Date(user.clientInfo.passportIssueDate).toLocaleDateString()}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {user.clientInfo.passportExpiryDate && (
                                                            <div className="flex items-start gap-2">
                                                                <FileText className="w-4 h-4 text-slate-400 mt-0.5 flex-shrink-0" />
                                                                <div>
                                                                    <p className="text-xs font-medium text-slate-500">Passport Expiry Date</p>
                                                                    <p className="text-sm text-slate-800">{new Date(user.clientInfo.passportExpiryDate).toLocaleDateString()}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {user.clientInfo.destination && (
                                                            <div className="flex items-start gap-2">
                                                                <MapPin className="w-4 h-4 text-slate-400 mt-0.5 flex-shrink-0" />
                                                                <div>
                                                                    <p className="text-xs font-medium text-slate-500">Destination</p>
                                                                    <p className="text-sm text-slate-800">{user.clientInfo.destination}</p>
                                                                </div>
                                                            </div>
                                                        )}
                                                        {user.clientInfo.note && (
                                                    ...
                                                    {isAdmin && (
                                                        <div className="flex gap-2 mt-4 pt-3 border-t border-slate-200">
                                                            <button
                                                                ...
                                                                className="flex items-center justify-center gap-1.5 px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg text-xs font-semibold transition-colors flex-1"
                                                            >
                                                            ...
                                                            <button
                                                                ...
                                                                className="flex items-center justify-center gap-1.5 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-xs font-semibold transition-colors flex-1"
                                                            >
