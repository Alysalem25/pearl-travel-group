...
const CategoriesPageContent = () => {
    ...
    const deleteImageMutation = useMutation({
        mutationFn: ({ id, name }: { id: string; name: string }) =>
            api.categories.deleteImage(id, name),
        ...
        try {
            if (editingCategory) {
                ...
                if (images.length > 0) {
                    ...
                    await api.categories.addImages(editingCategory._id, fd)
                }
    ...
    const removePreviewImage = (index: number) => {
        ...
        if (img.isNew) {
            ...
            if (!editingCategory || !img.name) return
    ...
    return (
        <div className="min-h-screen flex bg-white text-black">
            ...
            <div className="flex-1">
                ...
                {/* FORM */}
                {showForm && (
                    <form onSubmit={handleSubmit} className="m-6 p-6 bg-gray-200 rounded space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {previewImages.map((img, i) => (
                                    <div key={i} className="relative">
                                        ...
                                        <span className={`absolute bottom-1 left-1 text-xs px-2 rounded text-white ${img.isNew ? 'bg-yellow-600' : 'bg-blue-600'}`}>
                                            {img.isNew ? 'New' : 'Saved'}
                                        </span>
                                    </div>
                                ))}
                ...
                <div className="m-6">
                    {categories.map((c: Category) => (
                        <div key={c._id} className="p-4 bg-gray-800 text-white flex justify-between mb-2 rounded">
                            <span>{c.nameEn}</span>

                            <div className="flex gap-2">
                                <button onClick={() => startEdit(c)} className="bg-yellow-500 px-3 py-1 rounded">
                                    Edit
                                </button>

                                <button onClick={() => deleteMutation.mutate(c._id)} className="bg-red-600 px-3 py-1 rounded">
                                    Delete
                                </button>
                            </div>
                        </div>
