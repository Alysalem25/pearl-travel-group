...
//             <div className="flex-1 flex flex-col overflow-auto sjx'">
//                 {/* Modern Header */}
...
const FlightsPageContent = () => {
    ...
    const changeStatusMutation = useMutation({
        mutationFn: ({ id, status }: { id: string; status: string }) => 
            api.flights.changeStatus(id, status),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['flights'] })
    })
                                                ...
                                                <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                                                            ...
                                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-medium bg-indigo-50 text-indigo-700">
                                                                <span>{getTripTypeIcon(f.tripType)}</span>
                                                                {getTripTypeLabel(f.tripType)}
                                                            </span>
                                                            <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-600">
                                                                {getCabinLabel(f.cabinClass)}
                                                            </span>
                                                        </div>
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-600 mb-3">
                                                            ...
                                                            <span className="flex items-center gap-1.5">
                                                                ...
                                                                {f.numOfAdults} Adult{f.numOfAdults !== 1 ? 's' : ''}
                                                                {f.numOfChildren > 0 ? `, ${f.numOfChildren} Child${f.numOfChildren !== 1 ? 'ren' : ''}` : ''}
                                                            </span>
                                                            <span className="flex items-center gap-1.5">
                                                                <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                                                </svg>
                                                                Submitted {formatDateTime(f.createdAt)}
                                                            </span>
                                                        </div>

                                                        {/* Route Info - Round/OneWay */}
                                                        {f.tripType !== 'multi' && f.from && f.to && (
                                                            <div className="flex items-center gap-3 text-sm mb-2">
                                                                <span className="font-medium text-gray-900 bg-gray-100 px-3 py-1 rounded-lg">
                                                                    {f.from}
                                                                </span>
                                                                ...
                                                                <span className="text-gray-500 ml-2">
                                                                    {formatDate(f.date)}
                                                                    {f.tripType === 'round' && f.returnDate && (
                                                                        <> <span className="text-gray-400">→</span> {formatDate(f.returnDate)}</>
                                                                    )}
                                                                </span>
                                                            </div>
                                                        )}

                                                        {/* Route Info - Multi City */}
                                                        {f.tripType === 'multi' && f.multiCities && f.multiCities.length > 0 && (
                                                            <div className="space-y-2 mb-2">
                                                                {f.multiCities.map((city, idx) => (
